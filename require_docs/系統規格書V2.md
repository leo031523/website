...
| **安全** | 全站 HTTPS（nginx + Certbot）；JWT 存 HttpOnly cookie；密碼以 bcrypt/argon2 雜湊；輸入以 Pydantic 驗證；CORS 白名單；登入端點依 IP 做速率限制，防暴力破解。 |
...
- **速率限制**：`/api/auth/login` 依來源 IP 限制每分鐘 5 次、每日 50 次嘗試（不論帳密是否正確都計入），超過回傳 `429`，防暴力破解密碼；AI 聊天 API 另有獨立的每分鐘/每日額度（見 §9）。兩者共用同一套記憶體內滑動視窗實作（`app/core/rate_limit.py`），單一 process 規模不需要 Redis。
