...
> **文件版本**：v1.3（M0–M6 已實作完成，詳見 §13）
> **前端策略**：Next.js（App Router，公開頁 SSG/ISR）
> **反向代理**：nginx（搭配 Certbot / Let's Encrypt 管理 TLS）
> **視覺風格**：日式簡約（詳見 §6）
> **專案性質**：個人作品集網站，兼具技術筆記、作品展示與後台 CMS
> **狀態**：M0–M6 已上線運作，目前進行工程品質強化與 AI 助理功能規劃（見 `AI_FEATURE_REQUIREMENTS.md`）
...
- **USERS**：管理者帳號。系統設計為單一管理者，故此表通常只有一筆；採此設計仍保留日後擴充彈性。
- **ARTICLES**：文章/筆記。內容以 Markdown 原文（`content_md`）儲存，由前端伺服器端渲染。`status` 控制草稿/發布。
...
- **MEDIA**：上傳的圖片等媒體；文章與專案以 `cover_image_id` 參照作為封面。
...
- **認證**：JWT 存於 HttpOnly、Secure、SameSite cookie，前端 JavaScript 無法讀取，降低 XSS 竊取風險。
- **密碼**：以 bcrypt 或 argon2 雜湊儲存，絕不明文。
- **授權**：後台 API 以相依注入（dependency）統一驗證 JWT 與管理者身分。
- **輸入驗證**：所有請求以 Pydantic schema 驗證；slug 等使用者輸入做正規化。
- **CSRF**：對狀態變更請求採 SameSite cookie + CSRF token（視前後端同源策略而定）。
- **CORS**：限制允許來源為自身網域。
- **速率限制**：對 `/api/auth/login` 加上嘗試次數限制，防暴力破解（可於 nginx 層或應用層實作）。
- **Markdown 渲染防護**：渲染文章 Markdown 時需防範 XSS（停用或淨化原始 HTML，例如搭配 rehype-sanitize）。
- **機密管理**：資料庫密碼、JWT 密鑰、webhook token 以環境變數注入，不進版控（提供 `.env.example`）。
- **檔案上傳**：限制 MIME 類型與大小；儲存時重新命名，避免路徑穿越。
...
- **線框圖 / UI 設計稿**：首頁、文章頁、後台編輯頁的版面草圖（套用 §6 日式簡約風格）。
- **部署與維運手冊**：Docker Compose 設定、nginx.conf（反向代理 + 靜態快取）、Certbot 憑證流程、環境變數說明、備份還原步驟。
- **測試計畫**：測試策略與重點案例（認證、發布流程、權限）。
- **API 詳細 schema**：各端點完整的請求/回應欄位（可由 FastAPI OpenAPI 自動產生後補充）。
